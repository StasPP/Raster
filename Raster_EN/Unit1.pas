...
begin
  if OpenDialog1.Execute then
    try
      ...
      MessageDlg('File loading error: '+#13+OpenDialog1.FileName,mtError,[mbOk],1);
    end;
...
begin
 ...
 ExecAndWait(PChar(MyDir+'NomList_E.exe'),'-r', sw_restore);
 ...
 try
   ...
   Begin
     ...
     // GeoToUTM(WGS, B, L, false, y, x, zone, i=0);
...
begin
   ...
   0:
     begin
       StringGrid1.Cells[3,0] := 'Longtitude (L), ' +#176;
       StringGrid1.Cells[4,0] := 'Latitude (B), ' +#176;
     end;
   1,2:
     begin
       StringGrid1.Cells[3,0] := 'Easting, m';
       StringGrid1.Cells[4,0] := 'Northing, m';
     end;
   end;
...
begin
  if not OpenDialog2.Execute then
   ...
   if i= -1 then
   begin
     MessageDlg('Unknown Coordinate system or wrong format',mtError,[mbOk],0);
     exit;
   end;
...
//        Fname := OpenDialog1.FileName;
     ...
     ////////// Система координат
      ...
      begin
        S.Destroy;
        MessageDlg('Unknown Coordinate system', mtError,[mbOk],0);
        exit;
      end;
      ...
      i := 8;
      while not((GetCols(S[i],',',2,1, true)='')
            or(GetCols(S[i],',',2,1, true)=' '))or (i>S.Count-1) do
      try
         inc(i);
    ...
    MessageDlg('File Open Error', mtError, [mbOk],0);
  end;
 ...
 // FName := ExtractFilePath(Fname) + '\' + ImgFName;
  ...
  if i-9 >= 4 then
    if MessageDlg(' Compute Geo reference parameters? ', MtConfirmation, [mbYes, mbNo],0) = 6 then
      CountControl.Click;
...
begin
  ...
  Begin
    ...
    Memo1.Lines.Add('------');
    Memo1.Lines.Add('');
    Memo1.Lines.Add('Isolated control:');
    Memo1.Lines.Add('');
  End;
 ...
 //Form3.Memo1.Lines.Add('');
  ...
  //// Этап 5. Отчет
    ...
    //  RMS := RMS + Err*Err;
      ...
      Memo1.Lines.Add('   ' +  format('%n',[ Err ])+' m');
  ...
  //    Err := Err*aSF;
      ...
      Memo1.Lines.Add('   ' +  format('%n', [Err])+' pix.');
      Memo1.Lines.Add('');
...
//    End;
 ...
 For I := 0 To  PointCount -2 Do
  ...
  begin
    ...
    Memo1.Lines.Add('------');
    Memo1.Lines.Add('');
    Memo1.Lines.Add('Average values:');
    Memo1.Lines.Add('');
    Memo1.Lines.Add('  Scale: ' + format('%.4f',[aSF]) + ' Rotation angle: '+format('%.4f',[abeta*180/pi]));
    Memo1.Lines.Add('  Shift: X: ' + format('%n',[aC])+'  Y: '+ format('%n',[-aD])+
                    '  Coefficients: a: ' +  format('%.4f',[aa]) + '  b: ' +  format('%.4f',[ab]) );

    Memo1.Lines.Add('');
    Memo1.Lines.Add('------');
    Memo1.Lines.Add('');
    Memo1.Lines.Add('Referencing accuracy control:');
    Memo1.Lines.Add('');
  ...
  //    Err := Err*aSF;
    ...
    Memo1.Lines.Add('------');
    Memo1.Lines.Add('');
    Memo1.Lines.Add('RMS: '+ format('%n',[RMS*aSF])+' pix. ('+ format('%n',[RMS]) +' m)');
    Memo1.Lines.Add('');

    case trunc(RMS*aSF) of
      0..1:  Memo1.Lines.Add('Estimation: Good');
      2..4:  Memo1.Lines.Add('Estimation: Ok');
      5..10: Memo1.Lines.Add('Estimation: Low');
      11..25: Memo1.Lines.Add('Estimation: Bad');
      else
      begin
         Memo1.Lines.Add('There is big errors. Try again');
         GeoReady := false;
      end;
    end;
